...
## СТАТУС ИСПРАВЛЕНИЙ (2026-06-28, после триажа)

Исправлены: **#1, #2, #3, #4, #5, #6, #8, #10, #11, #12** (детали — `PROGRESS.md` → «Фиксы code-review»).
Регрессы покрыты `test/review-fixes.test.ts`. Гейт: 80 тестов ✓, `tsc` чист, `vite build` ок, живой smoke.
- **#7** — НЕ менялось (осознанный dev-дефолт баланса, см. memory `gen-project-decisions`).
- **#9** — частично: добавлена валидация формы сейва (`isValidCampaign`); QuotaExceeded-флаг в UI — вне scope.
- **efficiency/cleanup** (автосейв на каждый ход, грид O(W·H·U), useMemo, тройные хелперы cell-key/range+LoS) —
  НЕ трогались (перф-оптимизации, не баги; отдельная задача при необходимости).
